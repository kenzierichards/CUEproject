...
def main():
    ...
    #now stak has all links to results for desired years
    ...
    while stakResult: 
        ...
        #results tab
        eventResults = re.findall(r'Event Results[\s\S]*<pre>[\s\S]*', html) #all event results (including records, etc)
        ...
        for string in eventResults:
            event = re.split(r'(?=Girls)|(?=Boys)|(?=Women)|(?=Men)', string) #all event specific information
        ...
        #no duplicate events
        visitedEvent = []
        #every event + results (excluding relays)
        eventBodyList = []
        #every event + results without repeats
        finalEventBody = []
        ...
        for i in range (1, len(event)):
            ...
            #exclude relays, developmental events, etc
        ...
        #don't want prelims or duplicates
